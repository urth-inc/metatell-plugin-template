...
artifact, then apply it to a room. Do not improvise a generic React
...
- salt CSS Module class hashes with `versionId` to avoid cross-plugin clashes;
...
| Styles collide with the host/another plugin | Global CSS or unsalted CSS Module names | Keep styles local and retain the template's `versionId`-salted `generateScopedName`. |
